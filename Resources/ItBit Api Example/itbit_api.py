...
api_address = 'https://beta-api.itbit.com/v1'

class MessageSigner(object):

    def make_message(self, verb, url, body, nonce, timestamp=1435962975962):
 ...
 #       if nonce == 1:
  ...
  #      else:
   #         nonce = 2
  #          nonced_message = '2[\"POST\",\"https://beta-api.itbit.com/v1/wallets/118882b3-8102-427d-9546-b3baacc3ded1/orders/\",\"{\\\"type\\\": \\\"limit\\\", \\\"amount\\\": \\\"1.00\\\", \\\"price\\\": \\\"28.00\\\", \\\"side\\\": \\\"buy\\\", \\\"instrument\\\": \\\"XBTUSD\\\", \\\"currency\\\": \\\"XBT\\\"}\",\"2\",\"1435981157310\"]'
	
        print("nonced message: " + nonced_message)
        hash_digest = sha256_hash.digest()
        sha256_hash.update(nonced_message.encode('utf8'))
        hash_digest = sha256_hash.digest()
        print("url: " + url)
        print ("hash_digst: " + str(binascii.hexlify(hash_digest)))
        print("url.encode('utf8'): " + str(binascii.hexlify(url.encode('utf8') + hash_digest))); 
        hmac_digest = hmac.new(secret, url.encode('utf8') + hash_digest, hashlib.sha512).digest()
        print("hmac digest: " + str(binascii.hexlify(hmac_digest)))
        print("base64.b64encode: " + str(binascii.hexlify(base64.b64encode(hmac_digest))))
...
class itBitApiConnection(object):
    ...
    def make_request(self, verb, url, body_dict):
        ...
        print("Authorization: " + self.clientKey + ':' + signature.decode('utf8'))
        ...
        return requests.request(verb, url, data=json_body, headers=auth_headers, verify=False)
